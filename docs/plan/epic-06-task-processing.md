...
1. `JobDispatcher` (one per adapter id): registered as the stream's consumer; per job type a
   `JobHandler`; for a `WaitingJob` it calls `executor.tryAcquireSlot()`; with a slot, submit the
   handler; without one, count `jobs.dropped` and log at DEBUG (the lock lapses in 30 s and the engine
   hands the job out again, decision 9). A type without a handler is the WARN of S4.3.3.
2. `ZenBpmJobHandler(module, taskType, deployedProcesses, cache, rest, invoker, sharedValues, scoping,
   retry, drain)`:
...
3. Commands back to the engine go through `ZenBpmCommandRetry`: repeat what `isUnavailable` says
   (5 attempts, 50 ms doubling, bounded by the remaining lock = 30 s minus elapsed), stop at once
...
- `ZenBpmJobHandlerTest` (fake server + a `TestCollaborators` invoker): the context's fields, the
  ...
  the module skipped, a slot released on every path.
...
- COMPLETION_PENDING: nothing is sent; the job stays; the engine redelivers it every 30 s and the
  core answers each redelivery from the record with `COMPLETION_PENDING(openFor, maxAgeExceeded)`;
  where `maxAgeExceeded` and `async-task-max-age-action: incident` (new key, adapter level, default
  `report`) the handler fails the job without a code so an incident names the aggregate and the age.
  Every redelivery of an open task costs one slot for the duration of the record lookup; document it
  (GAPS 2, 3).

**Tests**: `ZenBpmTaskProcessingIT#aBpmnErrorRoutesTheBoundaryEvent`, `#anUnknownErrorCodeIsAnIncident`
(asserted through `GET .../incidents`), `#anAsynchronousTaskStaysOpen` (redelivered, handler runs once,
job still active after 70 s - use a shorter engine lock only if E13.1 landed; otherwise this test takes
70 s and is tagged `slow`), `ZenBpmJobHandlerTest` for the incident action.
...
### S6.2.1 Local retry, `max-redeliveries`, incident
...
handler throws (transaction rolled back), record the attempt with `notBefore = now +
retry-backoff * 2^(count-1)` capped at 5 minutes (resolved per module/workflow/task), send nothing
(decision 8), log at WARN with the exception type and message (an `NullPointerException` used to print
`null` in Camunda 8 - name the type). A redelivery arriving before `notBefore` is dropped without
running the handler (the lock lapses, the next one comes). At `count >= max-redeliveries`:
...
**Tests**: `ZenBpmLocalRetryTest` (backoff arithmetic, cap, drop-before-due, sweep);
`ZenBpmTaskProcessingIT#aThrowingHandlerEndsInAnIncidentAfterMaxRedeliveries` with
`max-redeliveries: 2` and `retry-backoff: PT1S` (about 70 s with the 30-second lock; tag `slow`).
...
but a thrown handler is not backed off or failed - the lock lapses. The factory's `close()` runs the
drain for any module which never stopped, with a WARN. A `shutdown-grace` above 30 s warns at startup
...
- Spring `ZenBpmInboundIdempotencyIT`: a handler which blocks 35 s on its first delivery; the second
  delivery (after the lock) runs concurrently and the core names both; after the first commits, a
  third delivery is answered from the record and the handler count stays at 2 (documented concurrent
  window, GAPS 2). A second case: a handler which completes but the completion command is made to
